"""POST /orders/{orderId}/cancel — cancel an order in an eligible state."""
...
def handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    if order.get("userId") != user_id:
        ...
        return error_from_app_error(
            AppError("FORBIDDEN", "You do not have access to this order", 403)
        )

    status = str(order.get("status", ""))
    if status in {"PAYMENT_PENDING", "FAILED"}:
        updated_at = now_iso()
        ok = repo.try_cancel_order(order_id, updated_at=updated_at)
        if not ok:
            log_structured(
                logger,
                "order_cancel_rejected",
                request_id=request_id,
                extra={"userId": user_id, "orderId": order_id, "status": status},
            )
            return error_from_app_error(
                AppError(
                    "INVALID_STATE",
        ...
        log_structured(
            ...
            "order_cancelled",
            request_id=request_id,
            extra={"userId": user_id, "orderId": order_id, "status": "CANCELLED"},
        )
        order = repo.get_order(order_id) or {**order, "status": "CANCELLED", "updatedAt": updated_at}
    ...
    return error_from_app_error(
        AppError(
            "INVALID_STATE",
            "Cancellation is not allowed for this order status",
            409,
        )
    )
